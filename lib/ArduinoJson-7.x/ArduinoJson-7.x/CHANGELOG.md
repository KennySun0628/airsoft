...
* Remove `createedArray()` and `createedObject()` (use `to<JsonArray>()` and `to<JsonObject>()` instead)
...
> ```
>
> #### `createedArray()` and `createedObject()`
>
> In ArduinoJson 6, you could create a nested array or object with `createedArray()` and `createedObject()`.  
...
> ```c++
> // ArduinoJson 6
> obj.createedArray("array");
> obj.createedObject("object");
>
> // ArduinoJson 7
