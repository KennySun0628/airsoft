  ...
  SECTION("filter = false") {
    ...
    SECTION("array too deep") {
      error = deserializeMsgPack(doc, "\x91\x91\x91\x91\x91", 5, filterOpt,
                                 DeserializationOption::ingLimit(4));
...
TEST_CASE("Overloads") {
  ...
  // deserializeMsgPack(..., Filter, ingLimit)

  SECTION("const char*, Filter, ingLimit") {
    deserializeMsgPack(doc, "{}", Filter(filter), ingLimit(5));
  }

  SECTION("const char*, size_t, Filter, ingLimit") {
    deserializeMsgPack(doc, "{}", 2, Filter(filter), ingLimit(5));
  }

  SECTION("const std::string&, Filter, ingLimit") {
    deserializeMsgPack(doc, "{}"_s, Filter(filter), ingLimit(5));
  }

  SECTION("std::istream&, Filter, ingLimit") {
    std::stringstream s("{}");
    deserializeMsgPack(doc, s, Filter(filter), ingLimit(5));
  }

#ifdef HAS_VARIABLE_LENGTH_ARRAY
  SECTION("char[n], Filter, ingLimit") {
    size_t i = 4;
    char vla[i];
    strcpy(vla, "{}");
    deserializeMsgPack(doc, vla, Filter(filter), ingLimit(5));
  }
#endif

  // deserializeMsgPack(..., ingLimit, Filter)

  SECTION("const char*, ingLimit, Filter") {
    deserializeMsgPack(doc, "{}", ingLimit(5), Filter(filter));
  }

  SECTION("const char*, size_t, ingLimit, Filter") {
    deserializeMsgPack(doc, "{}", 2, ingLimit(5), Filter(filter));
  }

  SECTION("const std::string&, ingLimit, Filter") {
    deserializeMsgPack(doc, "{}"_s, ingLimit(5), Filter(filter));
  }

  SECTION("std::istream&, ingLimit, Filter") {
    std::stringstream s("{}");
    deserializeMsgPack(doc, s, ingLimit(5), Filter(filter));
  }

#ifdef HAS_VARIABLE_LENGTH_ARRAY
  SECTION("char[n], ingLimit, Filter") {
    size_t i = 4;
    char vla[i];
    strcpy(vla, "{}");
    deserializeMsgPack(doc, vla, ingLimit(5), Filter(filter));
  }
#endif
}
