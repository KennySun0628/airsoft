  ...
  for (auto& tc : testCases) {
    SECTION(tc.description) {
      ...
      CHECK(deserializeJson(
                ...
                DeserializationOption::ingLimit(tc.nestingLimit)) ==
            tc.error);
...
TEST_CASE("Overloads") {
  ...
  // deserializeJson(..., Filter, ingLimit)

  SECTION("const char*, Filter, ingLimit") {
    deserializeJson(doc, "{}", Filter(filter), ingLimit(5));
  }

  SECTION("const char*, size_t, Filter, ingLimit") {
    deserializeJson(doc, "{}", 2, Filter(filter), ingLimit(5));
  }

  SECTION("const std::string&, Filter, ingLimit") {
    deserializeJson(doc, "{}"_s, Filter(filter), ingLimit(5));
  }

  SECTION("std::istream&, Filter, ingLimit") {
    std::stringstream s("{}");
    deserializeJson(doc, s, Filter(filter), ingLimit(5));
  }
  ...
  SECTION("char[n], Filter, ingLimit") {
    ...
    char vla[i];
    strcpy(vla, "{}");
    deserializeJson(doc, vla, Filter(filter), ingLimit(5));
  }
#endif

  // deserializeJson(..., ingLimit, Filter)

  SECTION("const char*, ingLimit, Filter") {
    deserializeJson(doc, "{}", ingLimit(5), Filter(filter));
  }

  SECTION("const char*, size_t, ingLimit, Filter") {
    deserializeJson(doc, "{}", 2, ingLimit(5), Filter(filter));
  }

  SECTION("const std::string&, ingLimit, Filter") {
    deserializeJson(doc, "{}"_s, ingLimit(5), Filter(filter));
  }

  SECTION("std::istream&, ingLimit, Filter") {
    std::stringstream s("{}");
    deserializeJson(doc, s, ingLimit(5), Filter(filter));
  }
  ...
  SECTION("char[n], ingLimit, Filter") {
    ...
    char vla[i];
    strcpy(vla, "{}");
    deserializeJson(doc, vla, ingLimit(5), Filter(filter));
  }
#endif
}
